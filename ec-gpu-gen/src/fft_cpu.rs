...
pub fn serial_fft<E: Engine>(a: &mut [E::Scalar], omega: &E::Scalar, log_n: u32) {
...
pub fn parallel_fft<E: Engine>(
    ...
    let mut tmp = vec![vec![E::Scalar::zero(); 1 << log_new_n]; num_threads];
...
mod tests {
    ...
    fn test_consistency<E: Engine, R: RngCore>(rng: &mut R) {
        let worker = Worker::new();
        for _ in 0..5 {
            for log_d in 0..10 {
                let d = 1 << log_d;
                let mut v1_coeffs =
                    (0..d).map(|_| E::Scalar::random(&mut *rng)).collect::<Vec<_>>();
                ...
                for log_threads in log_d..min(log_d + 1, 3) {
                    parallel_fft::<E>(&mut v1_coeffs, &worker, &v1_omega, log_d, log_threads);
                    serial_fft::<E>(&mut v2_coeffs, &v2_omega, log_d);
