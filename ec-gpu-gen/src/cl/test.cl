...
DEVICE Scalar32 vmx_double(Scalar32 a) {
    for(uchar i = Scalar32_LIMBS - 1; i >= 1; i--) {
        //printf("before: a.val[%d]: %d\n", i, a.val[i]);
        if (i == 7) {
            printf("a.val[i]: %d\n", a.val[i]);
            printf("a.val[i] << 1: %d\n", a.val[i] << 1);
            printf("a.val[i - 1] %d\n", a.val[i - 1]);
            printf("Scalar32_LIMB_BITS - 1: %d\n", Scalar32_LIMB_BITS - 1);
            printf("a.val[i - 1] >> (Scalar32_LIMB_BITS - 1: %d\n", a.val[i - 1] >> (Scalar32_LIMB_BITS - 1));
            printf("(a.val[i] << 1) | (a.val[i - 1] >> (Scalar32_LIMB_BITS - 1): %d\n", (a.val[i] << 1) | (a.val[i - 1] >> (Scalar32_LIMB_BITS - 1)));
        }

        a.val[i] = (a.val[i] << 1) | (a.val[i - 1] >> (Scalar32_LIMB_BITS - 1));
        //printf("after:  a.val[%d]: %d\n", i, a.val[i]);
    }
    a.val[0] <<= 1;
